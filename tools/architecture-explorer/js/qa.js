...
const qaTopics = [
...

  {
    ...
    plain:
      "API Gateway is the front door to your APIs: it receives requests, handles auth and throttling, and forwards them to backends like Lambda, containers, or on-premises services.",
    answer: [
      "Amazon API Gateway is a fully managed service for creating, securing, and publishing APIs.",
      "It supports REST, HTTP, and WebSocket APIs with built-in authentication, throttling, quotas, and request validation.",
      "Strengths:",
      "- No infrastructure to manage",
      "- Native integration with Lambda, ECS, and other AWS services",
      "- Good for exposing public and partner APIs safely",
      "Tradeoffs:",
      "- Adds a small latency hop between client and backend",
      "- It is an API layer, not a message broker - no ordering or replay",
      "Pair it with a queue or event stream (SQS, Kafka, Kinesis) when you need asynchronous processing.",
    ],
    match: [
      "api gateway",
      "amazon api gateway",
    ...
    terms: ["api gateway", "rest api", "websocket"],
    related: ["lambda", "which-architecture", "microservices"],
    example: "What is Amazon API Gateway?",
  },

  {
    ...
    topic: "AWS Lambda",
    plain:
      "Lambda runs a snippet of code whenever an event happens - an API call, a file upload, a queue message - with no servers to manage and no idle billing.",
    answer: [
      "AWS Lambda is event-driven compute: you upload code and it runs in response to triggers.",
      "Key characteristics:",
      "- Scales automatically, including to zero",
      "- Pay only for compute time used",
      "- Native triggers from S3, SQS, EventBridge, Kinesis, API Gateway, and more",
      "Tradeoffs:",
      "- Cold starts add latency for infrequent traffic",
      "- Execution time and package size limits",
      "- Stateless by design - persist state elsewhere",
      "Best for event handlers, API backends, and integration glue. For long-running, predictable, or resource-heavy workloads, containers (ECS, OpenShift) are often a better fit.",
    ],
    match: [
      "aws lambda",
      "lambda function",
    ...
    terms: ["lambda", "serverless function"],
    related: ["apigateway", "serverless-vs-containers", "sqs", "kinesis"],
...

  {
    ...
    plain:
      "Kinesis is a managed data stream: producers push records into shards and consumers read them back, with ordering per shard and records retained for replay.",
    answer: [
      "Amazon Kinesis Data Streams ingests and processes large streams of data in real time.",
      "Records are stored in shards; ordering is guaranteed within a shard, and retention is configurable (default 24h, up to 365 days) so consumers can replay.",
      "Strengths:",
      "- Fully managed sharded streaming",
      "- Per-shard ordering",
      "- Native integration with Lambda, Kinesis Analytics, and Flink",
      "Tradeoffs:",
      "- You plan shard capacity in advance and pay per shard",
      "- No consumer-group abstraction like Kafka - partitions map to shards",
      "Great for real-time analytics, log and telemetry pipelines, and IoT ingestion where Kafka would be overkill to operate.",
    ],
    match: [
      "amazon kinesis",
      "kinesis",
    ...
    terms: ["kinesis", "data stream", "streaming"],
    related: ["kafka", "event-streaming", "lambda", "which-architecture"],
    example: "What is Amazon Kinesis?",
  },

  {
    id: "containers-platforms",
    topic: "Containers Platforms (ECS, Kubernetes, OpenShift)",
    plain:
      "Container platforms run your services in portable units: ECS is AWS's managed option, Kubernetes gives you portability across clouds, and OpenShift adds enterprise governance on top.",
    answer: [
      "For long-running, portable workloads, containers are usually the answer:",
      "- AWS ECS: fully managed orchestration with Fargate (serverless) or EC2; simplest way to run containers on AWS",
      "- Kubernetes: portable across clouds and on-premises, huge ecosystem - but you operate the platform",
      "- Red Hat OpenShift on AWS (ROSA): enterprise Kubernetes with governance, security, and hybrid-cloud consistency, operated jointly by Red Hat and AWS",
      "Tradeoffs:",
      "- You manage images, capacity, and (for K8s/OpenShift) the platform itself",
      "- No native event replay - pair with a broker or data store for that",
      "Choose ECS for AWS-only simplicity, Kubernetes for portability, and OpenShift for regulated enterprises needing governance.",
    ],
    match: [
      "ecs",
    ...
    terms: ["ecs", "kubernetes", "openshift", "containers", "fargate", "rosa"],
    related: ["serverless-vs-containers", "lambda", "which-architecture"],
    example: "What is Red Hat OpenShift on AWS?",
  },

  {
    ...
    plain:
      "Pub/Sub is Google's global serverless messaging service: publishers push topics, subscribers pull or push, and messages can be re-read via seek.",
    answer: [
      "Google Cloud Pub/Sub is a managed messaging service for decoupling producers and consumers across regions.",
      "Key characteristics:",
      "- Serverless and globally distributed",
      "- Push and pull subscription modes",
      "- Seek to re-read messages (a form of replay)",
      "- Optional exactly-once delivery and ordering keys",
      "Tradeoffs:",
      "- Ordering requires explicit ordering keys and limits throughput",
      "- It sits outside the AWS ecosystem, so pair it with your own integration layer",
      "Good for global fan-out, multi-cloud, and serverless messaging when you are not locked into one vendor.",
    ],
    match: [
      "google pub/sub",
      "pub/sub",
    ...
    terms: ["pub/sub", "pubsub", "publish subscribe"],
    related: ["queue-vs-bus", "sqs", "which-architecture"],
    example: "What is Google Cloud Pub/Sub?",
  },
...

  {
    ...
    plain:
      "This tool scores 10 architectures - Kafka, SQS, EventBridge, RabbitMQ, API Gateway, Lambda, ECS, OpenShift (ROSA), Kinesis, and Pub/Sub - against your workload's needs and recommends the best fit. Pick a scenario above or describe what you're building.",
    answer: [
      ...
      "- Public APIs and integrations: Amazon API Gateway",
      "- Long-running containerized services: AWS ECS",
      "- Enterprise or hybrid Kubernetes: Red Hat OpenShift on AWS",
      "- Global serverless messaging: Google Cloud Pub/Sub",
